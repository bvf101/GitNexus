...
01 WS-CALL PIC X(8) VALUE 'GNXPGM01'.
01 WS-SOURCE PIC X(8).
01 WS-TARGET PIC X(8).
PROCEDURE DIVISION.
MAIN.
MOVE 'GNXPGM02' TO WS-SOURCE.
MOVE WS-SOURCE TO WS-TARGET.
CALL WS-TARGET.
MOVE 'GNXLATE1' TO WS-TARGET.
STOP RUN.`;
    ...
    expect(resolve('WS-CALL', callLine)).toEqual(['GNXPGM01']);
    expect(resolve('WS-TARGET', callLine)).toEqual(['GNXPGM02']);
    expect(resolve('WS-TARGET', source.split('\n').length + 1)).toEqual(['GNXPGM02', 'GNXLATE1']);
    expect(extracted.literalMoves).toEqual([
      expect.objectContaining({ value: 'GNXPGM02', targets: ['WS-SOURCE'] }),
      expect.objectContaining({ value: 'GNXLATE1', targets: ['WS-TARGET'] }),
    ]);
  });
...
MAIN.
IF WS-FLAG = 'A'
  MOVE 'GNXPGM01' TO WS-TARGET
ELSE
  MOVE 'GNXPGM02' TO WS-TARGET
END-IF
CALL WS-TARGET.`;
    ...
    expect(callLine).toBeGreaterThan(0);
    expect(resolve('WS-TARGET', callLine)).toEqual(['GNXPGM01', 'GNXPGM02']);
  });
...
EXEC CICS SEND MAP('GNXMAP01') END-EXEC.
EXEC CICS READ FILE(WS-FILE) END-EXEC.
EXEC CICS WRITEQ TS QUEUE('GNXQUE01') END-EXEC.`;
    ...
    expect(blocks[2]).toMatchObject({ mapName: 'GNXMAP01', mapIsLiteral: true });
    expect(blocks[3]).toMatchObject({ fileName: 'WS-FILE', fileIsLiteral: false });
    expect(blocks[4]).toMatchObject({ queueName: 'GNXQUE01', queueIsLiteral: true });
  });
});
